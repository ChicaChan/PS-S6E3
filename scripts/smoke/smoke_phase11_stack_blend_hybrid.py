# Usage:
# python scripts/smoke/smoke_phase11_stack_blend_hybrid.py \
#   --max-train-rows 4000 \
#   --max-test-rows 1500
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run minimal local smoke test for phase11 hybrid pipeline.")
...
def main() -> None:
    ...
    smoke_root = project_root / ".artifacts" / "smoke_phase11_hybrid"
    ...
    script_path = project_root / "kaggle_kernel" / "phase11_stack_blend_hybrid" / "hybrid_search.py"
    config_path = project_root / "kaggle_kernel" / "phase11_stack_blend_hybrid" / "hybrid_config_smoke.json"
    subprocess.run(
        [
            sys.executable,
            str(script_path),
    ...
    print("Phase11 hybrid smoke test passed.")
