# Usage:
# python scripts/smoke/smoke_phase8_catboost_strong.py \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Smoke test for phase8 CatBoost strong pipeline.")
...
def main() -> None:
    ...
    script_path = project_root / "kaggle_kernel" / "phase8_catboost_strong" / "train_catboost_strong.py"

    py_compile.compile(str(script_path), doraise=True)
    print("Py-compile passed for phase8 CatBoost strong pipeline.")

    if importlib.util.find_spec("catboost") is None:
        print("Skip runtime smoke for phase8: catboost is not installed locally.")
        return

    smoke_root = project_root / ".artifacts" / "smoke_phase8"
    ...
    config_path = input_dir / "smoke_phase8_config.json"
