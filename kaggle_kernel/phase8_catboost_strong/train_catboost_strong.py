...
#    python train_catboost_strong.py \
...
#      --config-path config_catboost_strong.json \
#      --output-dir /kaggle/working
#
# 2) Local smoke mode:
#    python train_catboost_strong.py \
#      --train-path .artifacts/smoke_phase8/input/train.csv \
#      --test-path .artifacts/smoke_phase8/input/test.csv \
#      --sample-submission-path .artifacts/smoke_phase8/input/sample_submission.csv \
#      --config-path .artifacts/smoke_phase8/input/smoke_phase8_config.json \
#      --output-dir .artifacts/smoke_phase8/output
...
def train_catboost(
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(outer_skf.split(np.zeros(len(train_df)), y), start=1):
        ...
        print(f"[CAT-STRONG] Fold {fold_idx}/{n_splits} AUC={fold_auc:.6f}, te_cols={len(te_cols)}")
...
def run_pipeline(
    ...
    if not ran_models:
        raise RuntimeError("No model was successfully trained in phase8.")
    ...
    report_path = output_dir / "phase8_report.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PS-S6E3 CatBoost strong-feature trainer")
    parser.add_argument(
        "--train-path",
        type=Path,
        required=False,
    ...
    parser.add_argument("--config-path", type=Path, required=False, default=Path("config_catboost_strong.json"))
