...
#    python train_lgbm_cat_diverse.py \
...
#      --config-path config_diverse_tree.json \
...
#      --config-path .artifacts/smoke/input/smoke_phase6_config.json \
#      --output-dir .artifacts/smoke/phase6_out
...
def read_config(config_path: Path) -> dict[str, Any]:
    ...
    merged.update({k: v for k, v in loaded.items() if k not in {"lgbm_params", "cat_params"}})
    merged["lgbm_params"] = DEFAULT_CONFIG["lgbm_params"].copy()
    merged["lgbm_params"].update(loaded.get("lgbm_params", {}))
...
def add_base_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["tenure", "MonthlyCharges", "TotalCharges", "service_count", "tenure_mod12"]:
        if col in out.columns:
            out[f"CAT_{col}"] = pd.to_numeric(out[col], errors="coerce").round(3).astype(str).replace("nan", "__MISSING__")
...
def add_orig_signal_features(
    ...
    for col in [c for c in top_cats if c in train.columns and c in orig_df.columns]:
    ...
    for col in ["tenure", "MonthlyCharges", "TotalCharges"]:
        ...
        tr_vals = pd.to_numeric(train[col], errors="coerce").fillna(np.nanmedian(orig_vals)).to_numpy(dtype=np.float32)
        te_vals = pd.to_numeric(test[col], errors="coerce").fillna(np.nanmedian(orig_vals)).to_numpy(dtype=np.float32)
...
def infer_prediction_column(df: pd.DataFrame, target_col: str) -> str:
    candidates = ["oof_prediction", "prediction", target_col, "pred"]
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(outer_skf.split(np.zeros(len(train_df)), y), start=1):
        ...
        try:
            ...
            model.fit(
                ...
                eval_metric="auc",
                callbacks=callbacks,
                categorical_feature=[c for c in x_train_all.columns if isinstance(x_train_all[c].dtype, pd.CategoricalDtype)],
            )
        except TypeError:
            model.fit(
                x_train_all,
        ...
        print(f"[LGBM] Fold {fold_idx}/{n_splits} AUC={fold_auc:.6f}, te_cols={len(te_cols)}")
...
def train_catboost(
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(outer_skf.split(np.zeros(len(train_df)), y), start=1):
        ...
        for c in final_cat_cols:
            ...
            x_val_all[c] = x_val_all[c].astype(str)
        ...
        test_pred += model.predict_proba(x_test_all)[:, 1].astype(np.float32) / n_splits
        fold_aucs.append(fold_auc)
        print(f"[CAT] Fold {fold_idx}/{n_splits} AUC={fold_auc:.6f}, te_cols={len(te_cols)}")
...
def run_pipeline(
    ...
    train_df, test_df = add_orig_signal_features(
        train_df=train_df,
        test_df=test_df,
        orig_df=orig_df,
        target_col=target_col,
    ...
    if bool(config.get("enable_lgbm", True)):
        try:
            oof_df, sub_df, metrics = train_lgbm(
                train_df=train_df,
                test_df=test_df,
                y=y,
            ...
            save_model_artifacts(output_dir, "lgbm", oof_df, sub_df, metrics)
            ran_models.append("lgbm")
            oof_frames.append(oof_df.rename(columns={"oof_prediction": "pred_lgbm"}))
            sub_frames.append(sub_df.rename(columns={target_col: "pred_lgbm"}))
        except Exception as exc:
            print(f"Skip LGBM due to error: {exc}")

    if bool(config.get("enable_catboost", True)):
        try:
            oof_df, sub_df, metrics = train_catboost(
                train_df=train_df,
                test_df=test_df,
                y=y,
                feature_cols=feature_cols,
            ...
            save_model_artifacts(output_dir, "cat", oof_df, sub_df, metrics)
            ran_models.append("cat")
            oof_frames.append(oof_df.rename(columns={"oof_prediction": "pred_cat"}))
            sub_frames.append(sub_df.rename(columns={target_col: "pred_cat"}))
        except Exception as exc:
            print(f"Skip CatBoost due to error: {exc}")

    if not ran_models:
        raise RuntimeError("No model was successfully trained in phase6.")
    ...
    print(f"Saved: {oof_ensemble_path}")
    print(f"Saved: {sub_ensemble_path}")
    print(f"Saved: {report_path}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PS-S6E3 diverse tree models (LGBM/CAT)")
    parser.add_argument(
        "--train-path",
        type=Path,
        required=False,
    ...
    parser.add_argument("--config-path", type=Path, required=False, default=Path("config_diverse_tree.json"))
