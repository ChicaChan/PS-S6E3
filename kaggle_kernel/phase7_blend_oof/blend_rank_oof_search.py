...
def blend_prob(matrix: np.ndarray, weights: np.ndarray) -> np.ndarray:
    ...
    out = np.average(matrix, axis=1, weights=w)
...
def blend_rank(matrix: np.ndarray, weights: np.ndarray) -> np.ndarray:
    ...
    out = np.average(rank_matrix, axis=1, weights=w)
    return np.clip(out, 0.0, 1.0)
...
def load_model_predictions(
    ...
    id_col: str,
    target_col: str,
) -> tuple[np.ndarray, pd.Series, np.ndarray, list[str], pd.DataFrame]:
    ...
    return y, base_oof_df[id_col], oof_matrix, names, pd.DataFrame({id_col: sample_ids, target_col: 0.0})
...
def search_best_weights(
    ...
    best_auc = -1.0
    best_weights = np.full(n_models, 1.0 / n_models, dtype=np.float64)
    best_method = "rank"

    for _ in range(max(1, n_trials)):
        weights = rng.dirichlet(np.ones(n_models, dtype=np.float64))
...
def run_blend(
    ...
    y, oof_ids, oof_matrix_all, names_all, sample_template = load_model_predictions(
        config=cfg,
    ...
    eq_weights = np.full(sel_oof.shape[1], 1.0 / sel_oof.shape[1], dtype=np.float64)
    ...
    # Load submission predictions again and keep selected columns in same order.
    base_dir = config_path.parent
    sample_df = pd.read_csv(sample_submission_path)
    sample_ids = sample_df[id_col].values
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PS-S6E3 OOF-constrained blend search")
