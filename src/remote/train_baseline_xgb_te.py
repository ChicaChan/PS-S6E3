...
def cast_categories_consistently(
    ...
    for col in cat_cols:
        train_s = x_train[col].astype("string").fillna("__MISSING__")
        val_s = x_val[col].astype("string").fillna("__MISSING__")
        test_s = x_test[col].astype("string").fillna("__MISSING__")
        categories = pd.Index(pd.concat([train_s, val_s, test_s], axis=0).unique(), dtype="string")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config-path", type=Path, required=False, default=Path("src/remote/config_baseline.json"))
